...
import Dogfood from '../../../../public/pet supplies/dogfood.jpg'
import { api } from '~/trpc/server';
import Link from 'next/link';


export default async function PetSuppliesCard() {

  const Products = await api.product.getProducts();
  const petSupplies = Products.filter(product => product.category === 'Petsupplies')
      ...
      {petSupplies.map((product) => (
        <div key={product.id}>
          <li className="relative flex flex-row items-start border-2 rounded p-2 mt-2">
            ...
            <Image src={Dogfood} alt="" className="p-4 ml-10 flex-shrink-0 shadow-lg rounded w-full sm:w-[10rem] xl:mb-2" width="1280" height="200" />
