...
export default async function AluminumCard() {
  ...
  const aluminums = products.filter(product => product.category === 'Aluminum')
  return (
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4'>
        {aluminums?.map((aluminum) => (
          <Link
            ...
            href={`/admin/customers/${aluminum.id}`}>
            <h2 className="font-mono font-bold text-lg">{aluminum.name}</h2>
            <p className="text-sm"> - {aluminum.name}</p>
